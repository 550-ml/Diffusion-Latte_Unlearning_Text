...
class Latte(nn.Module):
    ...
    def forward(self, x, t, y=None, use_fp16=False, y_image=None, use_image_num=0):
        ...
        if self.extras == 2:
            y = self.y_embedder(y, self.training)  # 经过编码
            if self.training:
                y_image_emb = []
                # print(y_image)
                for y_image_single in y_image:
                    # print(y_image_single)
                    y_image_single = y_image_single.reshape(1, -1) # （1，32）
            ...
            text_embedding = self.text_embedding_projection(text_embedding)# [b, t, d]
            text_embedding_video = text_embedding[:, :1, :]# [b, 1, d]
            text_embedding_image = text_embedding[:, 1:, :]# [b, t-1,d]
...
if __name__ == '__main__':
    ...
    use_image_num = 8 
    # 创建一个video
    img = torch.randn(3, 16+use_image_num, 4, 32, 32).to(device) # b,f,c,h,w
