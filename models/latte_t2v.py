...
class LatteT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if self.caption_projection is not None:
            ...
            if use_image_num != 0 and self.training:
                ...
                print(f'Latte2tv:{encoder_hidden_states_spatial[0,:2,:10]}')
    ...
    def get_sentence_embedding(
        ...
        if self.is_input_patches: # here
            ...
            self.pos_embed = self.pos_embed.to(hidden_states.dtype)
            hidden_states = self.pos_embed(hidden_states) # alrady add positional embeddings
        ...
        if self.caption_projection is not None:
            batch_size = hidden_states.shape[0]
            encoder_hidden_states = encoder_hidden_states.to(torch.float32) # 3 120 1152
            encoder_hidden_states = self.caption_projection(encoder_hidden_states) # 3 120 1152

            if use_image_num != 0 and self.training:
                encoder_hidden_states_video = encoder_hidden_states[:, :1, ...]
                encoder_hidden_states_video = repeat(encoder_hidden_states_video, 'b t d -> b (f t) d', f=frame).contiguous()
                print(f'encoder_hidden_states_video{encoder_hidden_states_video}')
                encoder_hidden_states_image = encoder_hidden_states[:, 1:, ...] # b 119 1152
                encoder_hidden_states = torch.cat([encoder_hidden_states_video, encoder_hidden_states_image], dim=1)
                encoder_hidden_states_spatial = rearrange(encoder_hidden_states, 'b (f t) d -> (b f) t d').contiguous()  # b f t d
            else:
                encoder_hidden_states_spatial = repeat(encoder_hidden_states, 'b t d -> (b f) t d', f=frame).contiguous()
        ...
        return encoder_hidden_states_spatial, encoder_attention_mask
